	...
	function UnitPauseTarget( caster,target,pausetime)
		if (target:GetContext("ability_stunsystem_pause_duration")==nil) then
		...
		if(old_stun_duration==0)then
			...
			--寻找单位释放技能
			local STUN_TARGET = dummy:FindAbilityByName("ability_stunsystem_pause")
			
			dummy:CastAbilityOnTarget(target, STUN_TARGET, 0 )
			target:SetContextThink('ability_stunsystem_pause',
				function ()
					local systemStunDuration = target:GetContext("ability_stunsystem_pause_duration")
					if(systemStunDuration>0)then
						systemStunDuration = systemStunDuration - 0.1
						target:SetContextNum("ability_stunsystem_pause_duration",systemStunDuration,0)
						return 0.1
					end
					target:SetContextNum("ability_stunsystem_pause_duration",0,0)
