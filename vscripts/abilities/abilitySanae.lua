...
function OnSanae01SpellStart(keys)
	...
	Timer.Loop 'OnSanae01SpellStart' (0, keys.Duration,
		function(i)
	    	...
	    	for _,v in pairs(targets) do
				local damage_table = {
					victim = v,
					attacker = caster,
					damage = keys.ability:GetAbilityDamage()-300,
...
function OnSanae04SpellStart(keys)
	...
	ParticleManager:SetParticleControlEnt(effectIndex, 0, caster, 0, follow_origin, caster:GetOrigin(), false)
...
function OnSanae04Invulnerable(keys)
	...
	for _,v in pairs(targets) do
		if(v:GetContext("ability_sanea_04_invulnerable")~=TRUE)then
			...
			ParticleManager:SetParticleControlEnt(unitEffectIndexUnit, 0, v, 0, follow_origin, v:GetOrigin(), false)
